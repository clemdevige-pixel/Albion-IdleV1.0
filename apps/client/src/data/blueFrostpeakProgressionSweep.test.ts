...
type Checkpoint = { readonly id: string; readonly segmentIndex: number; readonly mastery: number; readonly enchantment: Enchantment; readonly useHealthPotions: boolean };
...
describe("Blue Frostpeak runtime progression sweep", () => {
  it("prints the T4.1-to-T4.2 final Blue progression probes", () => {
    const rows = CHECKPOINTS.flatMap((checkpoint) => T4_WEAPONS.map((weaponItemId) => {
      const result = runCombatRuntimeBenchmark({ label: checkpoint.id, weaponItemId, zoneDefId: WORLD_ZONE_IDS.mountain, segmentIndex: checkpoint.segmentIndex, equipmentItemIds: equipmentFor(weaponItemId), masteryLevel: checkpoint.mastery, enchantment: checkpoint.enchantment, useHealthPotions: checkpoint.useHealthPotions });
    ...
    console.table(rows); console.log("[BLUE_FROSTPEAK_RUNTIME_PROGRESSION_SWEEP]", JSON.stringify(rows, null, 2));
