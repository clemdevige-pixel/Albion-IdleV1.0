...
function equipmentFor(itemId: string, checkpoint: Checkpoint): readonly string[] {
  if (checkpoint.mode === "full_t3") {
    const items: string[] = [...T3_ARMOR];
    if (resolveEquipmentInfo(itemId)?.handling === "one_handed") items.push(T3_SHIELD);
    return items;
  }
  ...
  if (resolveEquipmentInfo(itemId)?.handling === "one_handed") items.push(T4_SHIELD);
  return items;
}
...
describe("Blue Highlands runtime progression sweep", () => {
  it("prints the T3-to-T4 transition probes", () => {
    const rows = CHECKPOINTS.flatMap((checkpoint) => {
      const items = checkpoint.tier === 3 ? T3 : T4;
      return items.map((weaponItemId) => {
    ...
    console.table(rows);
    console.log("[BLUE_HIGHLAND_RUNTIME_PROGRESSION_SWEEP]", JSON.stringify(rows, null, 2));
