...
const T3_WEAPONS = ["item_weapon_sword_t3_broadsword", "item_weapon_bow_t3_longbow", "item_weapon_staff_t3_infernal", "item_weapon_gloves_t3_spiked_gauntlets", "item_weapon_dagger_t3_pair"] as const;
...
type Checkpoint = { readonly id: string; readonly zoneDefId: (typeof WORLD_ZONE_IDS)[keyof typeof WORLD_ZONE_IDS]; readonly segmentIndex: number; readonly weaponTier: 3 | 4; readonly masteryLevel: number; readonly enchantment: 0 | 1 | 2 | 3; readonly armor: "none" | "torso_t3" | "full"; readonly useHealthPotions: boolean };
...
function shortWeaponName(itemId: string): string { return itemId.replace("item_weapon_", "").replace("_t3_", " ").replace("_t4_", " "); }
function equipmentFor(weaponItemId: string, checkpoint: Checkpoint): readonly string[] {
  ...
  if (resolveEquipmentInfo(weaponItemId)?.handling === "one_handed") armor.push(checkpoint.weaponTier === 3 ? T3_SHIELD : T4_SHIELD);
  return armor;
}
describe("Blue live runtime progression matrix", () => {
  it("prints every checkpoint through the exact live combat runtime", () => {
    const rows = CHECKPOINTS.flatMap((checkpoint) => (checkpoint.weaponTier === 3 ? T3_WEAPONS : T4_WEAPONS).map((weaponItemId) => {
      const result = runCombatRuntimeBenchmark({ label: checkpoint.id, weaponItemId, zoneDefId: checkpoint.zoneDefId, segmentIndex: checkpoint.segmentIndex, equipmentItemIds: equipmentFor(weaponItemId, checkpoint), enchantment: checkpoint.enchantment, masteryLevel: checkpoint.masteryLevel, useHealthPotions: checkpoint.useHealthPotions });
      return { checkpoint: checkpoint.id, weapon: shortWeaponName(weaponItemId), clear: result.clear, seconds: result.seconds, hpPercent: result.hpPercent, potions: result.potionsUsed, encounters: result.encounterReached, hp: result.maxHealth, armor: result.armor, mr: result.magicResistance, mastery: result.masteryLevel };
    }));
    console.table(rows); console.log("[BLUE_LIVE_RUNTIME_PROGRESSION_MATRIX]", JSON.stringify(rows, null, 2));
    expect(rows).toHaveLength(CHECKPOINTS.length * 5); expect(rows.every((row) => Number.isFinite(row.seconds))).toBe(true); expect(rows.every((row) => row.encounters >= 1 && row.encounters <= 5)).toBe(true);
  });
});
