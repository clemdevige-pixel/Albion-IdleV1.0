...
describe("same-tier .3 dungeon benchmark across all weapons", () => {
  it("runs every authored faction dungeon with every live weapon family", () => {
    const rows: Array<{
      tier: Tier;
      faction: string;
      dungeon: string;
      ...
      for (const dungeon of dungeons) {
        for (const family of FAMILIES) {
          const result = runCombatRuntimeBenchmark({
            label: `${dungeon.id}:${family}:t${tier}.3`,
    ...
    const weaponSummary = FAMILIES.flatMap((weapon) => TIERS.map((tier) => {
      const weaponRows = rows.filter((row) => row.weapon === weapon && row.tier === tier);
      const cleared = weaponRows.filter((row) => row.clear);
      return {
        tier,
        weapon,
        clears: `${cleared.length}/${weaponRows.length}`,
        avgSeconds: cleared.length > 0 ? round1(cleared.reduce((sum, row) => sum + row.seconds, 0) / cleared.length) : 0,
        avgHpPct: cleared.length > 0 ? round1(cleared.reduce((sum, row) => sum + row.hpPercent, 0) / cleared.length) : 0,
    ...
    console.log("[DUNGEON_TN3_ALL_WEAPONS_BENCHMARK]");
    console.table(rows);
    console.log("[DUNGEON_TN3_TIER_SUMMARY]");
    console.table(tierSummary);
    console.log("[DUNGEON_TN3_WEAPON_SUMMARY]");
    console.table(weaponSummary);
    console.log("[DUNGEON_TN3_ALL_WEAPONS_JSON]", JSON.stringify(rows, null, 2));

    expect(rows).toHaveLength(DUNGEON_DEFINITIONS.length * FAMILIES.length);
