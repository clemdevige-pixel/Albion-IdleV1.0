 ...
 * Values are authored as final rates, not runtime multipliers, so the Bestiary
 * can display the exact probability used by combat rolls for each depth.
 */
export const FACTION_RUNE_WORLD_DROP_RATES: Readonly<
  Record<WorldBandId, readonly FactionRuneWorldDropZoneRate[]>
> = {
  // Blue contains three T3 zones first. Runes begin only on the T4 portion.
  blue: [
    { start: 0, end: 0 },
    { start: 0, end: 0 },
    { start: 0, end: 0 },
  ...
  black: [
    ...
    // Blackspire is not yet farmable by the calibration profile. Do not invent
    // an unvalidated higher cap: it inherits the current T8 tester ceiling.
