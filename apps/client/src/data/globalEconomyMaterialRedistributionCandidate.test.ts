...
const SHARD_RATE_MULTIPLIERS = [1, 1.1, 1.2, 1.3, 1.4, 1.5] as const;

// Final diagnostic candidate for the common armor craft mix.
// Total base material counts stay equal to the current live recipes; only family distribution changes.
const FINAL_COMMON_ARMOR: Readonly<Record<TargetTier, Readonly<Record<Slot, Readonly<Record<Family, number>>>>>> = {
  4: {
    head: { wood: 1, ore: 4, hide: 2, fiber: 1 },
    chest: { wood: 2, ore: 1, hide: 3, fiber: 3 },
    boots: { wood: 2, ore: 1, hide: 2, fiber: 1 },
  },
  5: {
    head: { wood: 2, ore: 5, hide: 2, fiber: 1 },
    chest: { wood: 2, ore: 2, hide: 4, fiber: 3 },
    boots: { wood: 2, ore: 1, hide: 3, fiber: 2 },
  },
  6: {
    head: { wood: 2, ore: 6, hide: 2, fiber: 2 },
    chest: { wood: 3, ore: 2, hide: 4, fiber: 4 },
    boots: { wood: 3, ore: 1, hide: 3, fiber: 3 },
  },
  7: {
    head: { wood: 3, ore: 7, hide: 2, fiber: 2 },
    chest: { wood: 3, ore: 3, hide: 5, fiber: 4 },
    boots: { wood: 3, ore: 2, hide: 4, fiber: 3 },
  },
  8: {
    head: { wood: 3, ore: 8, hide: 3, fiber: 2 },
    chest: { wood: 4, ore: 3, hide: 5, fiber: 5 },
    boots: { wood: 4, ore: 2, hide: 4, fiber: 4 },
  },
};
...
function tick(state: State, heroActive: boolean, demandTier = state.tier, demand?: Record<Family, number>): void {
  state.ticks += 1;
  if (state.ticks > MAX_TICKS) throw new Error("candidate diagnostic runaway");
...
function commonItemIds(tier: TargetTier): readonly string[] {
  return [`item_helmet_t${tier}_reinforced`, `item_armor_t${tier}_leather`, `item_boots_t${tier}_leather`];
}
function profileItemIds(tier: TargetTier, profile: WeaponProfile): readonly string[] {
  const weapon = weaponId(tier, profile);
  const items = [weapon, ...commonItemIds(tier)];
  if (resolveEquipmentInfo(weapon)?.handling === "one_handed") items.push(`item_shield_t${tier}_reinforced`);
  return items;
}
function slotForCommon(itemId: string): Slot | undefined {
  if (itemId.includes("helmet")) return "head";
  if (itemId.includes("armor_t")) return "chest";
...
function candidateCraftCost(tier: TargetTier, profile: WeaponProfile): Record<Family, number> {
  ...
  for (const itemId of profileItemIds(tier, profile)) {
    ...
    addCosts(out, slot === undefined ? liveRecipeCosts(itemId) : FINAL_COMMON_ARMOR[tier][slot]);
  }
  return out;
}
...
function candidateEnchantCost(tier: TargetTier, profile: WeaponProfile, level: 1 | 2 | 3): { shards: number; refined: Record<Family, number> } {
  ...
  for (const itemId of profileItemIds(tier, profile)) {
    ...
    const slot = slotForCommon(itemId);
    const craftMaterials = slot === undefined ? info.craftMaterials : craftMaterialsFromCosts(tier, FINAL_COMMON_ARMOR[tier][slot]);
...
function baseShardRate(tier: TargetTier, profile: WeaponProfile, enchantment: Enchantment): number {
  ...
  for (const zone of ZONES[tier]) {
    for (let segment = 0; segment < 10; segment += 1) {
      const result = runCombatRuntimeBenchmark({
        label: `candidate_${profile}_T${tier}.${enchantment}_${zone}_${segment}`,
        weaponItemId: weapon,
...
function setGatherTier(state: State, tier: ProductionTier): void {
  for (const f of FAMILIES) {
    switchHero(state.families[f], tier);
    switchWorker(state.families[f], tier);
  }
}
...
function runProfile(profile: WeaponProfile, shardRateMultiplier: number): ProfileRow[] {
  const state = createState();
  const rows: ProfileRow[] = [];
  for (const tier of [4, 5, 6, 7, 8] as const) {
    prepareTransition(state, (tier - 1) as ProductionTier, tier);
    state.tier = tier;
    setGatherTier(state, tier);

    const craft = candidateCraftCost(tier, profile);
    while (!spend(state, tier, craft)) tick(state, true, tier, craft);

    let shardHours = 0;
    let gatherHours = 0;
    let shards = 0;
    const enchant = emptyCosts();
    const bottlenecks = new Set<Family>();

    for (const level of [1, 2, 3] as const) {
      const cost = candidateEnchantCost(tier, profile, level);
      addCosts(enchant, cost.refined);
      shards += cost.shards;
      const rate = baseShardRate(tier, profile, (level - 1) as Enchantment) * shardRateMultiplier;
      if (rate <= 0) throw new Error(`No shard farm ${profile} T${tier}.${level - 1}`);
      ...
      const before = Object.fromEntries(FAMILIES.map((f) => [f, maxProducible(state.families[f], tier)])) as Record<Family, number>;
      for (const f of FAMILIES) if (before[f] < cost.refined[f]) bottlenecks.add(f);
      if (!FAMILIES.every((f) => before[f] >= cost.refined[f])) {
    ...
    rows.push({
      ...
      enchantRefined: formatCosts(enchant),
      totalShards: shards,
      shardFarmHours: round2(shardHours),
      extraHeroGatherHours: round2(gatherHours),
      combinedHours: round2(combined),
      gatherSharePct: combined > 0 ? round1(gatherHours / combined * 100) : 0,
...
describe("global economy final material mix + shard rate sweep", () => {
  it("keeps the final craft mix fixed and measures shard drop-rate candidates", () => {
    const rows = SHARD_RATE_MULTIPLIERS.flatMap((multiplier) => WEAPONS.flatMap((profile) => runProfile(profile, multiplier)));
    const summary = SHARD_RATE_MULTIPLIERS.flatMap((multiplier) => [4, 5, 6, 7, 8].map((tier) => {
      const tierRows = rows.filter((row) => row.shardRateMultiplier === multiplier && row.tier === `T${tier}`);
      ...
      return {
        shardRateMultiplier: multiplier,
        tier: `T${tier}`,
        avgCombinedHours: round2(avgCombined),
        avgGatherSharePct: round1(avgGatherShare),
        minGatherSharePct: round1(minGatherShare),
        maxGatherSharePct: round1(maxGatherShare),
        bottlenecks,
      };
    }));

    console.log("[GLOBAL_ECONOMY_FINAL_CRAFT_MIX]");
    console.table(Object.entries(FINAL_COMMON_ARMOR).flatMap(([tier, slots]) => Object.entries(slots).map(([slot, costs]) => ({ tier: `T${tier}`, slot, ...costs }))));
    console.log("[GLOBAL_ECONOMY_SHARD_RATE_SWEEP_SUMMARY]");
    console.table(summary);
    console.log("[GLOBAL_ECONOMY_SHARD_RATE_SWEEP_DETAIL]");
    console.table(rows);
    console.log("[GLOBAL_ECONOMY_SHARD_RATE_SWEEP_SUMMARY_JSON]", JSON.stringify(summary, null, 2));

    expect(rows).toHaveLength(SHARD_RATE_MULTIPLIERS.length * WEAPONS.length * 5);
    expect(summary).toHaveLength(SHARD_RATE_MULTIPLIERS.length * 5);
  });
});
