...
export const MONSTER_DEFINITIONS: Readonly<Record<string, MonsterContentDefinition>> = {
  ...
  [MONSTER_IDS.hereticThug]: { id: MONSTER_IDS.hereticThug, name: "Heretic Thug", faction: "Heretic", category: "normal", tier: 4, visualManifestId: "monster_heretic_thug", combat: { damageType: "physical" }, rewards: { lootTableId: "loot_monster_generic" }, abilityIds: [], tags: ["heretic", "melee", "thug"] },
  [MONSTER_IDS.hereticFirestarter]: { id: MONSTER_IDS.hereticFirestarter, name: "Heretic Firestarter", faction: "Heretic", category: "normal", tier: 4, visualManifestId: "monster_heretic_firestarter", combat: { damageType: "magical" }, rewards: { lootTableId: "loot_monster_generic" }, abilityIds: [], tags: ["heretic", "fire", "ranged"] },
  [MONSTER_IDS.hereticEnforcer]: { id: MONSTER_IDS.hereticEnforcer, name: "Heretic Enforcer", faction: "Heretic", category: "elite", tier: 4, visualManifestId: "monster_heretic_enforcer", combat: { damageType: "physical" }, rewards: { lootTableId: "loot_monster_generic" }, abilityIds: [], tags: ["heretic", "elite", "melee", "segment_boss"] },
...
export function resolveMonsterForEncounter(zoneDefId: ZoneDefinitionId, segmentIndex: number, encounterIndex: number, random: () => number = Math.random): MonsterContentDefinition { const pool = getZoneEncounterPool(zoneDefId); const isBoss = encounterIndex === ENCOUNTERS_PER_SEGMENT - 1; const isBiomeBoss = isBoss && segmentIndex === SEGMENTS_PER_ZONE - 1; if (isBiomeBoss) return getMonsterDefinition(pool.biomeBoss); if (isBoss) return getMonsterDefinition(pool.segmentBoss); const index = Math.min(pool.normal.length - 1, Math.floor(Math.max(0, random()) * pool.normal.length)); const monsterId = pool.normal[index] ?? pool.normal[0]; if (monsterId === undefined) throw new Error(`Zone has no normal monsters: ${String(zoneDefId)}`); return getMonsterDefinition(monsterId); }
