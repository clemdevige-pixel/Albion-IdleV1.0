...
import { PLAYER_ISLAND_CONFIG, getIslandBuildingDefinition, getIslandLevelDefinition, type IslandBuildingId } from "@game/data";
...
interface T3OnboardingRow { readonly targetItemId: string; readonly workshopFamilies: string; readonly buildings: string; readonly silverCost: number; readonly rawWood: number; readonly rawOre: number; readonly rawHide: number; readonly rawFiber: number; readonly totalRaw: number; readonly gatherSecondsHeroOnly: number; readonly gatherSecondsHeroPlusOneWorker: number; }
...
function buildScenario(targetItemId: string): T3OnboardingRow {
  ...
  const rawTotals: Record<ProductionFamilyId, number> = { wood: 0, ore: 0, hide: 0, fiber: 0 }; let silverCost = 0;
  for (const buildingId of requiredBuildings) { const construction = getIslandBuildingDefinition(buildingId).construction; if (construction === undefined) continue; silverCost += construction.silver; for (const requirement of construction.requirements) addRawEquivalent(rawTotals, requirement); }
  ...
  return { targetItemId, workshopFamilies: workshopFamilies.join(" + "), buildings: [...requiredBuildings].join(" -> "), silverCost, rawWood: rawTotals.wood, rawOre: rawTotals.ore, rawHide: rawTotals.hide, rawFiber: rawTotals.fiber, totalRaw, gatherSecondsHeroOnly: Math.round(totalRaw / heroPerSecond * 10) / 10, gatherSecondsHeroPlusOneWorker: Math.round(totalRaw / (heroPerSecond + workerPerSecond) * 10) / 10 };
}

describe("T3 onboarding economy benchmark", () => {
  it("prints the current data-driven cost and gather-time envelope for the first T3 craft", () => { const rows = T3_TARGETS.map(buildScenario); console.table(rows); console.info("[T3_ONBOARDING_BENCHMARK]", JSON.stringify({ runtimeTickRate: createRuntimeServices().config.tickRate, heroT3GatherCycleTicks: getProductionTierRules(3).gatheringBaseTicks, workerT3GatherCycleTicks: WORKER_TASK_DEFINITIONS.find((task) => task.resourceTier === 3)?.durationTicks, rows }, null, 2)); expect(rows).toHaveLength(T3_TARGETS.length); expect(rows.every((row) => row.totalRaw > 0)).toBe(true); expect(rows.every((row) => row.silverCost > 0)).toBe(true); });
