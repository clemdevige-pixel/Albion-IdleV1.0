    ...
    mechanics: {
      ...
      mechanics: deathgiversOriginal.mechanics.mechanics.map((mechanic: any) => (
        mechanic.kind === "damage" && mechanic.bonusEffect?.effectId === "effect_dagger_opening"
          ? { ...mechanic, bonusEffect: { ...mechanic.bonusEffect, bonusRatio: 2.50 } }
          : mechanic
      )),
    },
  };
...
const TRIAL_SEED = "tower-endless-final-micro-pass-trial";
...
function runWeaponBlock(block: BenchmarkBlock, weapon: BenchmarkSpec, seed: string) {
  ...
  try {
    const result = runCombatRuntimeBenchmark({
      label: `tower_final_micro_${weapon.label.replaceAll(" ", "_").toLowerCase()}_t${String(tier)}_${String(block.blockIndex + 1)}`,
      weaponItemId,
...
describe("Tower Endless final micro-pass benchmark", () => {
  it("validates the last micro-pass before live authoring", () => {
    const rows = runAll();
    ...
    console.log("[TOWER_FINAL_MICRO_FAVORABLE_WEAPON_TIER]");
    console.table(favorable);
    console.log("[TOWER_FINAL_MICRO_FAVORABLE_FAILURES]");
    console.table(favorableFailures);
    console.log("[TOWER_FINAL_MICRO_NEUTRAL_LEAKS_BY_TIER]");
    console.table(neutralLeaks);
    console.log("[TOWER_FINAL_MICRO_NEUTRAL_LEAK_WEAPONS]");
    console.table(neutralLeakWeapons);
    console.log("[TOWER_FINAL_MICRO_T8_FAVORABLE]");
    console.table(t8Favorable);
    console.log("[TOWER_FINAL_MICRO_DAGGER_SUMMARY]");
    console.table(daggerSummary);
    console.log("[TOWER_FINAL_MICRO_DEATHGIVERS]");
    console.table(deathgivers);
    console.log("[TOWER_FINAL_MICRO_NOTE] Benchmark-only. T8 extra x1.05. Galatine 2.90; Whispering AA window 0.60 and vulnerability removed; Wildfire 1.45; Great Fire 3.55; Ursine finisher 2.50; Battle Bracers 3.75; Deathgivers Opening +2.50; Claws 1.03x4 + bleed 0.10. All prior accepted Tower and weapon changes retained.");
