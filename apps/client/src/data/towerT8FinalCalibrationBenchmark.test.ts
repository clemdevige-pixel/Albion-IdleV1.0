...
const TRIAL_SEED = "tower-t8-final-calibration-trial";
const T8_PRESETS = [1.0, 1.2, 1.25, 1.3, 1.35] as const;
    ...
    return ARTIFACT_WEAPON_BENCHMARK_SPECS.map((weapon) => {
      ...
      try {
        const result = runCombatRuntimeBenchmark({
          label: `tower_t8_final_${String(preset)}_${weapon.label.replaceAll(" ", "_").toLowerCase()}_${String(block.blockIndex + 1)}`,
          weaponItemId,
        ...
        return { preset, weapon: weapon.label, faction, matchup, clear: result.clear, hpPct: result.hpPercent };
      } finally {
...
describe("Tower T8 final calibration benchmark", () => {
  it("tests final weapon candidates against T8-only difficulty presets", () => {
    ...
    console.log("[TOWER_T8_FINAL_PRESET_SUMMARY]");
    console.table(summary);
    console.log("[TOWER_T8_FINAL_FAVORABLE_WEAPON]");
    console.table(perWeapon);
    console.log("[TOWER_T8_FINAL_FAVORABLE_FAILURES]");
    console.table(failures);
    console.log("[TOWER_T8_FINAL_NOTE] Benchmark-only. Dagger sustain removed family-wide; Deathgivers Opening bonus +1.8x; Great Fire 3.3x; Ursine finisher 2.25x; Claws 1.0x4 + bleed 0.10; Battle Bracers 3.5x; Badon 1.05x; Blazing 1.35x3; Whispering vulnerability removed. T8 preset multiplies only the already-calibrated faction×tier combat profile.");
