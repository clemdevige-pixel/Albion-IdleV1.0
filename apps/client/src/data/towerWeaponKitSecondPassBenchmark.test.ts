...
const bloodletterDiagnosticOverride = vi.hoisted(() => ({
  disableSharedDaggerSustain: false,
}));
...
// Candidate-only Whispering change: remove +15% self damage-taken vulnerability.
// Ratios, attack-speed buff, auto-attack bonus window, duration and cooldown remain authored.
...
// Bloodletter diagnostic-only override. When enabled, remove only the heal_from_damage
// mechanic from shared Double Slash. Damage, cooldown and every other dagger mechanic remain live.
vi.mock("./weaponContentCatalog.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveUnlockedWeaponAbilities: (weaponItemId: string, familyMasteryLevel: number) => {
      ...
      if (!bloodletterDiagnosticOverride.disableSharedDaggerSustain || !weaponItemId.includes("dagger_bloodletter")) {
        return abilities;
      }
...
const TRIAL_SEED = "tower-weapon-kit-second-pass-trial";
...
// Second-pass benchmark-only weapon candidates.
const DIRECT_ABILITY_TUNING: Readonly<Record<string, number>> = {
  ability_bow_badon_raging_storm: 1.15 / 1.35,
  ability_dagger_demonfang_blood_ritual: 1.30 / 2.0,
  ability_dagger_claws_disembowel: 1.0 / 1.4,
  ability_fire_blazing_flame_tornado: 1.35 / 1.5,
  ability_fire_great_pyroblast: 2.7 / 2.0,
  ability_gloves_battle_bracers_falcon_smash: 3.3 / 2.6,
};
const EFFECT_DAMAGE_TUNING_BY_WEAPON: Readonly<Record<string, number>> = {
  "Claws": 0.12 / 0.15,
};
// Ursine finisher 1.65 -> 1.90. Harness applies one multiplier to the direct signature packet,
// so preserve the earlier weighted proxy: authored 1.8 multi-hit component + 1.65 finisher.
...
function runWeaponBlock(
  ...
  seed: string,
  options?: { readonly removeShield?: boolean; readonly disableBloodletterSustain?: boolean; readonly labelSuffix?: string },
) {
  ...
  const authoredEquipment = artifactDungeonEquipment(weaponItemId, tier, dungeon.faction);
  const equipmentItemIds = options?.removeShield === true
    ? authoredEquipment.filter((itemId) => !itemId.startsWith("item_shield_"))
    : authoredEquipment;
  bloodletterDiagnosticOverride.disableSharedDaggerSustain = options?.disableBloodletterSustain === true;

  try {
    const result = runCombatRuntimeBenchmark({
      label: `tower_weapon_second_pass_${weapon.label.replaceAll(" ", "_").toLowerCase()}_t${String(tier)}_${String(block.blockIndex + 1)}${options?.labelSuffix ?? ""}`,
      weaponItemId,
      equipmentItemIds,
    ...
    bloodletterDiagnosticOverride.disableSharedDaggerSustain = false;
  }
}
...
const BLOODLETTER_VARIANTS = [
  { id: "current", removeShield: false, disableSustain: false },
  { id: "no_shield", removeShield: true, disableSustain: false },
  { id: "no_sustain", removeShield: false, disableSustain: true },
  { id: "no_shield_no_sustain", removeShield: true, disableSustain: true },
] as const;

function runBloodletterT8NeutralDiagnostic() {
  const bloodletter = ARTIFACT_WEAPON_BENCHMARK_SPECS.find((weapon) => weapon.label === "Bloodletter");
  if (bloodletter === undefined) throw new Error("Missing Bloodletter benchmark spec");
  const t8NeutralBlocks = allBlocks().filter(({ block }) => {
    if (block.tier !== 8 || !isFaction(block.factionId)) return false;
    const weaponItemId = bloodletter.itemId(8);
    const modifiers = resolveFactionCombatModifiers(
      { weaponItemId, capeItemId: `item_cape_t8_${block.factionId}` },
      { factionId: block.factionId, tier: 8, activity: "tower" },
    );
    return modifiers.outgoingDamageBonusPercent === 0;
  });

  return BLOODLETTER_VARIANTS.map((variant) => {
    const rows = t8NeutralBlocks.map(({ block, seed }) => runWeaponBlock(block, bloodletter, seed, {
      removeShield: variant.removeShield,
      disableBloodletterSustain: variant.disableSustain,
      labelSuffix: `_${variant.id}`,
    }));
    const clears = rows.filter((row) => row.clear);
    return {
      variant: variant.id,
      shield: !variant.removeShield,
      daggerSustain: !variant.disableSustain,
      runs: rows.length,
      clears: clears.length,
      leakRatePct: Number(((clears.length / rows.length) * 100).toFixed(1)),
      avgClearHpPct: clears.length === 0
        ? 0
        : Number((clears.reduce((sum, row) => sum + row.hpPct, 0) / clears.length).toFixed(1)),
      avgDamageReceived: Number((rows.reduce((sum, row) => sum + row.damageReceived, 0) / rows.length).toFixed(1)),
      avgPotionsUsed: Number((rows.reduce((sum, row) => sum + row.potionsUsed, 0) / rows.length).toFixed(2)),
      deepestClearedFloor: clears.length === 0 ? null : Math.max(...clears.map((row) => row.floorEnd)),
    };
  });
}

describe("Tower weapon kit second-pass benchmark", () => {
  it("tests validated weapon adjustments and Bloodletter defensive-source diagnostics together", () => {
    const rows = runAll();
    ...
    const bloodletterDiagnostic = runBloodletterT8NeutralDiagnostic();

    console.log("[TOWER_WEAPON_SECOND_PASS_FAVORABLE_WEAPON_TIER]");
    console.table(favorable);
    console.log("[TOWER_WEAPON_SECOND_PASS_FAVORABLE_FAILURES]");
    console.table(favorableFailures);
    console.log("[TOWER_WEAPON_SECOND_PASS_NEUTRAL_LEAKS_BY_TIER]");
    console.table(neutralLeaksByTier);
    console.log("[TOWER_WEAPON_SECOND_PASS_NEUTRAL_LEAK_WEAPONS]");
    console.table(neutralLeakWeapons);
    console.log("[TOWER_BLOODLETTER_T8_NEUTRAL_DEFENSE_DIAGNOSTIC]");
    console.table(bloodletterDiagnostic);
    console.log("[TOWER_WEAPON_SECOND_PASS_NOTE] Benchmark-only second pass on faction x tier candidate matrix. Badon 1.35->1.15; Demonfang 2.0x3->1.30x3; Claws 1.4x4->1.0x4 and bleed stays candidate 0.12; Blazing stays 1.35x3; Great Fire 2.0->2.7; Battle Bracers 2.6->3.3; Ursine finisher stays candidate 1.90 weighted proxy; Whispering removes only +15% self damage-taken vulnerability. Bloodletter itself is unchanged in the main benchmark. Its T8 neutral diagnostic independently tests shield and shared Double Slash sustain removal without changing execution damage.");

    expect(rows.length).toBeGreaterThan(0);
    expect(favorable.length).toBeGreaterThan(0);
    expect(bloodletterDiagnostic).toHaveLength(4);
  }, 300_000);
});
