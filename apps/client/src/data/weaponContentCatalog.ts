...
export type WeaponCraftRule =
  | { readonly kind: "standard"; readonly family: "bow" | "sword" | "fire_staff" | "gloves"; readonly materials: readonly WeaponCraftMaterial[] }
  | { readonly kind: "artifact_pending"; readonly family: "bow" };
...
const WEAPON_CONTENT: readonly WeaponSpecializationContent[] = [
  {
    familyMasteryId: "mastery_sword", familyName: "Épées", specializationMasteryId: "mastery_broadsword", specializationName: "Épée large", combatProfile: "sword",
    craft: { kind: "standard", family: "sword", materials: [{ kind: "metal", quantity: 6 }, { kind: "leather", quantity: 2 }] },
  ...
  {
    familyMasteryId: "mastery_bow", familyName: "Arcs", specializationMasteryId: "mastery_longbow", specializationName: "Arc long", combatProfile: "bow",
    craft: { kind: "standard", family: "bow", materials: [{ kind: "wood", quantity: 6 }, { kind: "leather", quantity: 2 }, { kind: "cloth", quantity: 2 }] },
  ...
  {
    familyMasteryId: "mastery_bow", familyName: "Arcs", specializationMasteryId: "mastery_badon", specializationName: "Badon", combatProfile: "bow",
    // Artifact crafting is intentionally not simulated until artifact content exists.
    craft: { kind: "artifact_pending", family: "bow" },
    ability: { id: "ability_bow_aimed_shot", name: "Tir ajusté", description: "Un tir précis infligeant 160 % des dégâts physiques.", icon: "🏹", category: "active", cooldown: 5, castTime: 0, resourceCost: { energy: 8 }, interruptible: true, targetRule: "current_target", damageType: "physical", bonusDamageRatio: 0.6 },
    items: [{ itemId: "item_weapon_bow_t4_badon", tier: 4, handling: "two_handed", stats: { stat_physical_damage: 87 }, sellPrice: 260 }],
  },
  {
    familyMasteryId: "mastery_fire_staff", familyName: "Bâtons de feu", specializationMasteryId: "mastery_t4_fire_staff", specializationName: "Bâton de feu", combatProfile: "staff",
    craft: { kind: "standard", family: "fire_staff", materials: [{ kind: "wood", quantity: 4 }, { kind: "metal", quantity: 4 }, { kind: "cloth", quantity: 2 }] },
  ...
  {
    familyMasteryId: "mastery_gloves", familyName: "Gants", specializationMasteryId: "mastery_spiked_gauntlets", specializationName: "Gantelets à pointes", combatProfile: "gloves",
    craft: { kind: "standard", family: "gloves", materials: [{ kind: "metal", quantity: 5 }, { kind: "leather", quantity: 3 }] },
...
export function resolveWeaponMastery(itemId: string): WeaponMasteryRoute | undefined { const entry = CONTENT_BY_ITEM_ID.get(itemId)?.specialization; if (entry === undefined) return undefined; return { familyId: asMasteryId(entry.familyMasteryId), weaponId: asMasteryId(entry.specializationMasteryId) }; }
...
const masteryNames = new Map<string, string>(); const familyMasteryIds = new Set<string>(); const specializationMasteryIds = new Set<string>();
for (const entry of WEAPON_CONTENT) { masteryNames.set(entry.familyMasteryId, entry.familyName); masteryNames.set(entry.specializationMasteryId, entry.specializationName); familyMasteryIds.add(entry.familyMasteryId); specializationMasteryIds.add(entry.specializationMasteryId); }
