...
export type AbilityAutoCastRule =
...
const ABILITIES = {
  ...
  broadswordExecution: {
    ...
    autoCast: { kind: "target_health_below", ratio: 0.3 },
  },
  bowAimedShot: {
...
export const WEAPON_FAMILIES = {
  sword: {
    ...
    name: "Épées",
    sharedAbilities: [ABILITIES.swordHeroicStrike, ABILITIES.swordGuardBreaker],
  },
  bow: {
    masteryId: "mastery_bow",
    name: "Arcs",
    sharedAbilities: [ABILITIES.bowAimedShot, ABILITIES.bowPiercingArrow],
  },
  fire_staff: {
    masteryId: "mastery_fire_staff",
    name: "Bâtons de feu",
    sharedAbilities: [ABILITIES.fireFireball, ABILITIES.fireInfernalBurst],
  },
  gloves: {
    masteryId: "mastery_gloves",
    name: "Gants",
    sharedAbilities: [ABILITIES.glovesShockwave, ABILITIES.glovesBreakingCombo],
  },
  dagger: {
    masteryId: "mastery_dagger",
    name: "Dagues",
    sharedAbilities: [ABILITIES.daggerDoubleSlash, ABILITIES.daggerFlurry],
  },
} as const;
...
const WEAPON_CONTENT: readonly WeaponSpecializationContent[] = [
  {
    ...
    signatureAbility: ABILITIES.broadswordExecution,
  ...
  },
  {
    ...
    signatureAbility: ABILITIES.longbowDeadeye,
  ...
  },
  {
    ...
    combatProfile: "bow",
    presentation: { itemIcon: "item-badon-pixel-v1.png", actorManifestId: "hero_bow", combatProfileId: "projectile", combatPresentation: { kind: "projectile", projectileId: "badon_arrow", releaseDelayMs: 355 } },
    signatureAbility: ABILITIES.badonRagingStorm,
  ...
  },
  {
    ...
    signatureAbility: ABILITIES.infernalCataclysm,
  ...
  },
  {
    ...
    signatureAbility: ABILITIES.spikedSeismicImpact,
  ...
  },
  {
    ...
    signatureAbility: ABILITIES.daggerPairAssassination,
    craft: { kind: "standard", materials: [{ kind: "metal", quantity: 6 }, { kind: "leather", quantity: 2 }] },
    items: [
...
export const CLIENT_ABILITIES: Readonly<Record<string, ClientAbilityDefinition>> = Object.fromEntries(
  Object.values(ABILITIES).map((ability) => [ability.id, ability] as const),
);
...
export const WEAPON_VENDOR_OFFERS = WEAPON_CONTENT.flatMap((entry) =>
  entry.items.map((item) => ({ itemId: item.itemId, buyPrice: null, sellPrice: item.sellPrice, maxPerTransaction: null, enabled: true })),
);
