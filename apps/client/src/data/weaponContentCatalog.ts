...
const CONTENT_BY_ITEM_ID=new Map<string,WeaponItemRoute>(WEAPON_CONTENT.flatMap(s=>s.items.map(item=>[item.itemId,{specialization:s,item}] as const)));
export function resolveWeaponAbilityUnlocks(itemId:string|null|undefined):readonly WeaponAbilityUnlock[]{if(itemId==null)return[];const s=CONTENT_BY_ITEM_ID.get(itemId)?.specialization;if(!s)return[];const f=WEAPON_FAMILIES[s.familyId];const[q,w]=f.sharedAbilities;return[...(q?[{unlockMasteryLevel:1,source:"family" as const,ability:q}]:[]),...(w?[{unlockMasteryLevel:10,source:"family" as const,ability:w}]:[]),{unlockMasteryLevel:30,source:"specialization" as const,ability:s.signatureAbility}];}
export function resolveUnlockedWeaponAbilities(itemId:string|null|undefined,specializationMasteryLevel:number):readonly ClientAbilityDefinition[]{return resolveWeaponAbilityUnlocks(itemId).filter(x=>specializationMasteryLevel>=x.unlockMasteryLevel).sort((a,b)=>a.unlockMasteryLevel-b.unlockMasteryLevel).map(x=>x.ability);}
...
export function resolveWeaponMastery(itemId:string):WeaponMasteryRoute|undefined{const e=CONTENT_BY_ITEM_ID.get(itemId)?.specialization;if(!e)return;const f=WEAPON_FAMILIES[e.familyId];return{familyId:asMasteryId(f.masteryId),weaponId:asMasteryId(e.specializationMasteryId)};}
...
export function getWeaponMasteryFamilyDefinitions():readonly WeaponMasteryFamilyDefinition[]{return(Object.keys(WEAPON_FAMILIES) as WeaponFamilyId[]).map(familyId=>({familyId,masteryId:WEAPON_FAMILIES[familyId].masteryId,specializationMasteryIds:WEAPON_CONTENT.filter(e=>e.familyId===familyId).map(e=>e.specializationMasteryId)}));}
const masteryNames=new Map<string,string>();const familyMasteryIds=new Set<string>();const specializationMasteryIds=new Set<string>();for(const f of Object.values(WEAPON_FAMILIES)){masteryNames.set(f.masteryId,f.name);familyMasteryIds.add(f.masteryId);}for(const e of WEAPON_CONTENT){masteryNames.set(e.specializationMasteryId,e.specializationName);specializationMasteryIds.add(e.specializationMasteryId);}
...
export const WEAPON_VENDOR_OFFERS=WEAPON_CONTENT.flatMap(e=>e.items.map(item=>({itemId:item.itemId,buyPrice:null,sellPrice:item.sellPrice,maxPerTransaction:null,enabled:true})));
