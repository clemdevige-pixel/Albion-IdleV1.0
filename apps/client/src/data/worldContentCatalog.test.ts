...
describe("worldContentCatalog", () => {
  ...
  it("generates the complete authored progression chain without special cases", () => {
    ...
    expect(ZONE_UNLOCK_DEFINITIONS.at(-1)?.zoneDefId).toBe(WORLD_ZONE_IDS.doompeak);
    expect(ZONE_UNLOCK_DEFINITIONS.at(-1)?.conditions).toEqual([{ type: "zone_completed", targetZoneDefId: WORLD_ZONE_IDS.crimsonSteppe }]);
  });
  it("links a future band generically to the previous authored zone", () => {
    const futureZone = asZoneDefinitionId("zone_future_stage_1_t8"); const definitions = buildZoneUnlockDefinitions([...WORLD_ZONE_ORDER, futureZone]);
    expect(definitions.at(-1)).toEqual({ zoneDefId: futureZone, conditions: [{ type: "zone_completed", targetZoneDefId: WORLD_ZONE_IDS.doompeak }] });
  });
  it("links Yellow, Orange and Red without special cases", () => {
