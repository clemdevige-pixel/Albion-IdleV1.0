...
describe("world/island tier progression", () => {
  ...
  it("uses Ironveil completion for both Orange access and island level 4", () => {
    const cinderwoodUnlock = ZONE_UNLOCK_DEFINITIONS.find(
      (definition) => definition.zoneDefId === WORLD_ZONE_IDS.cinderwood,
    );

    expect(cinderwoodUnlock?.conditions).toContainEqual({
      type: "zone_completed",
      targetZoneDefId: WORLD_ZONE_IDS.ironveil,
    });
    expect(getIslandLevelDefinition(4)?.worldRequirementToReach).toEqual({
