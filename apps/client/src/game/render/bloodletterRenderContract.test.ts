...
const FRAME_COUNT = 6;
const SOURCE_CHARACTER_HEIGHT_PX = 388;
const TEXTURE_KEY = "hero-bloodletter-attack-normalized-v1";
const ASSET_PATH = "/assets/characters/hero-bloodletter-attack-normalized-v1.png";

describe("bloodletter render contract", () => {
  it("routes every Bloodletter tier to its dedicated actor manifest", () => {
    const specialization = FACTION_ARTIFACT_WEAPON_CONTENT.find(
      (entry) => entry.specializationMasteryId === "mastery_bloodletter",
    );
    expect(specialization).toBeDefined();

    for (const item of specialization?.items ?? []) {
      expect(resolveEquipmentPresentation(item.itemId)?.actorManifestId).toBe("hero_bloodletter");
    }
  });

  it("reuses frame 0 as idle and the full authored sheet as attack", () => {
    const manifest = renderManifestRegistry.requireActor("hero_bloodletter");
  ...
  it("keeps the committed runtime sheet at six normalized cells", () => {
    const assetUrl = new URL(
      "../../../public/assets/characters/hero-bloodletter-attack-normalized-v1.png",
      import.meta.url,
    );
