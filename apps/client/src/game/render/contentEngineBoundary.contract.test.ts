...
function readEngineSources(): readonly {
  readonly path: string;
  readonly source: string;
}[] {
  return ENGINE_FILES.map((url) => ({
...
describe("content / engine architectural boundary", () => {
  it("keeps every declared weapon render route resolvable without engine-specific branches", () => {
    for (const itemId of Object.keys(WEAPON_ITEM_DEFINITIONS)) {
      const presentation = resolveWeaponPresentation(itemId);

      expect(presentation, `${itemId}: presentation`).toBeDefined();
      ...
      if (presentation.combatPresentation?.kind === "projectile") {
        expect(
          renderManifestRegistry.requireProjectile(
            presentation.combatPresentation.projectileId,
          ),
  ...
  it("keeps every declared monster render route resolvable without engine-specific branches", () => {
    for (const monster of Object.values(MONSTER_DEFINITIONS)) {
      expect(
        renderManifestRegistry.getStaticActor(monster.visualManifestId),
        `${monster.id}: static actor manifest ${monster.visualManifestId}`,
      ).toBeDefined();
    }
  });
  ...
  it("keeps concrete content ids out of GameScene and presentation systems", () => {
    ...
    for (const { path, source } of sources) {
      for (const id of forbiddenIds) {
        expect(
          source.includes(id),
          `${path} must not hardcode content id "${id}"`,
        ).toBe(false);
      }
    }
  });
