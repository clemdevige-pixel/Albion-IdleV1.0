...
const FRAME_COUNT = 6;
const MIN_FEET_Y = 574;
const MAX_FEET_Y = 575;

const ASSET_PATHS = [
  "/assets/characters/hero-great-fire-attack-normalized-v1.png",
  "/assets/characters/hero-infernal-attack-normalized-v1.png",
  "/assets/characters/hero-wildfire-attack-normalized-v1.png",
  "/assets/characters/hero-blazing-attack-normalized-v1.png",
  "/assets/characters/hero-brimstone-attack-normalized-v1.png",
] as const;
...
describe("normalized fire staff sprite assets", () => {
  for (const assetPath of ASSET_PATHS) {
    it(`keeps six meaningful frames and the shared baseline for ${assetPath}`, () => {
      ...
      for (let frameIndex = 0; frameIndex < FRAME_COUNT; frameIndex += 1) {
        ...
        expect(visiblePixels, `${assetPath} frame ${String(frameIndex)} visible pixels`).toBeGreaterThan(30_000);
        expect(opaquePixels, `${assetPath} frame ${String(frameIndex)} opaque pixels`).toBeGreaterThan(20_000);
        expect(maxAlpha, `${assetPath} frame ${String(frameIndex)} max alpha`).toBeGreaterThanOrEqual(250);
        expect(maxY, `${assetPath} frame ${String(frameIndex)} feet baseline`).toBeGreaterThanOrEqual(MIN_FEET_Y);
        expect(maxY, `${assetPath} frame ${String(frameIndex)} feet baseline`).toBeLessThanOrEqual(MAX_FEET_Y);
      }
    });
  }
});
