...
export function AbilityBar(): JSX.Element {
  ...
  return (
    <div className="ability-bar">
      <div className="ability-bar__skills" aria-label="Compétences de combat">
        <button
          type="button"
      ...
      <div className="ability-bar__status">
        <button
          type="button"
          ...
          disabled={ability === null}
          onClick={() => {
            actions.setPrimaryAbilityAutoCast(!(ability?.autoCast ?? false));
          }}
        >
          <span>AUTO</span>
          <i aria-hidden="true" />
        ...
        <div className="ability-bar__energy" title="Énergie disponible">
          <span className="ability-bar__energy-label">Énergie</span>
          <strong>
            {String(Math.round(model.currentEnergy))}
            <span>/</span>
            {String(Math.round(model.maxEnergy))}
          </strong>
          <span className="ability-bar__energy-track" aria-hidden="true">
            <i
              style={{
                width: `${String(model.maxEnergy <= 0
                  ? 0
                  : Math.min(100, Math.max(0, (model.currentEnergy / model.maxEnergy) * 100)))}%`,
              }}
            />
          </span>
        </div>
        ...
        <div className="ability-bar__potion-wrapper">
          ...
          <div className="ability-tooltip ability-tooltip--potion" role="tooltip">
            ...
            <div className="ability-tooltip__stats">
              <span>
                Soin
                <strong>{String(model.potionHealPercent)}% PV max</strong>
              </span>
              <span>
                Cooldown
                <strong>{String(model.potionCooldown)} s</strong>
              </span>
              <span>
                Quantité
