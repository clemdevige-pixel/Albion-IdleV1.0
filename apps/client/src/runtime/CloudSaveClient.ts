...
export class CloudSaveClient {
  ...
  public async list(): Promise<CloudSaveList> { const response = await fetch(this.endpoint(CLOUD_SAVES_ROUTE), { headers: this.headers() }); if (!response.ok) throw new Error("Cloud save list failed"); return CloudSaveListSchema.parse(await response.json()); }
  public async get(slotId: CloudSaveSlotId): Promise<CloudSaveDocument | undefined> { const response = await fetch(this.endpoint(`${CLOUD_SAVES_ROUTE}/${slotId}`), { headers: this.headers() }); if (response.status === 404) return undefined; if (!response.ok) throw new Error("Cloud save download failed"); return CloudSaveDocumentSchema.parse(await response.json()); }
  public upload(slotId: CloudSaveSlotId, document: CloudSaveDocument): Promise<void> {
    const operation = this.uploadQueue.then(async () => {
      const response = await fetch(this.endpoint(`${CLOUD_SAVES_ROUTE}/${slotId}`), { method: "PUT", headers: this.headers(true), body: JSON.stringify(document) });
  ...
  public async delete(slotId: CloudSaveSlotId): Promise<void> { const response = await fetch(this.endpoint(`${CLOUD_SAVES_ROUTE}/${slotId}`), { method: "DELETE", headers: this.headers() }); if (!response.ok && response.status !== 404) throw new Error("Cloud save deletion failed"); }
}
