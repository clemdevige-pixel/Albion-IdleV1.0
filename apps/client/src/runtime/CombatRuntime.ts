...
export class CombatRuntime {
  ...
  public spawnEnemy(): SpawnedEnemyResult {
    ...
    return spawnEnemyForSegment(this.combatEntityFactoryDeps, this.biomeResolver, {
      zoneIndex: loc.zoneIndex,
      segmentIndex: loc.segmentIndex,
      encounterIndex: loc.encounterIndex,
      zoneDefId: loc.zoneDefId,
  ...
  public initialize(): CombatDomainTickResult {
    ...
    for (const definition of Object.values(CLIENT_ABILITIES)) {
      this.abilityManager.learnAbility(this.heroId, definition);
    }

    const encounterResult = this.combatService.startEncounter(
      {
        id: asEncounterId(`encounter_${String(this.encounterCounter)}`),
  ...
  public getActiveEnemyId(): EntityId {
    return this.activeEnemyId;
  }

  public isAutoCastEnabled(): boolean {
    return this.primaryAbilityAutoCast;
  }

  public setPrimaryAbilityAutoCast(enabled: boolean): void {
    this.primaryAbilityAutoCast = enabled;
  }

  public isAwaitingResumeAfterDefeat(): boolean {
    return this.awaitingResumeAfterDefeat;
  }

  public restoreHeroHealth(): void {
    const health = this.damageManager.getHealth(this.heroId);
    this.damageManager.healDamage(
      this.heroId,
  ...
  public interruptEncounter(): void {
    ...
    if (session !== undefined) {
      ...
      for (const enemyId of session.participants.enemies) {
        ...
        if (this.world.hasEntity(enemyId)) {
  ...
  public resumeExploration(): boolean {
    if (!this.awaitingResumeAfterDefeat) {
      return false;
    }
  ...
  public usePrimaryAbility(): boolean {
    ...
    const execution = this.abilityManager.executeIntent({
      entityId: this.heroId,
      abilityId: abilityId as AbilityId,
      primaryTarget: this.activeEnemyId,
  ...
  public tick(dt: number, tickCounter: number): CombatDomainTickResult {
    ...
    if (this.ports.isCombatSuspended()) {
    ...
    if (session === undefined) {
      ...
      return {
        ...
        activeEnemy: {
          ...
          maxHealth: enemyHealth.maxHealth,
          name: enemy.name,
          visualManifestId: enemy.visualManifestId,
        },
        playerHealth: {
          currentHealth: heroHealth.currentHealth,
          maxHealth: heroHealth.maxHealth,
    ...
    for (const [, effects] of this.orchestrator.getState().activeEffects) {
      for (const eff of effects) {
        activeEffects.push({
          id: eff.id,
          definitionId: eff.definition.id,
    ...
    const enemyHealth = this.damageManager.isAlive(this.activeEnemyId)
      ? this.damageManager.getHealth(this.activeEnemyId)
      : undefined;
    const heroHealth = this.damageManager.getHealth(this.heroId);
