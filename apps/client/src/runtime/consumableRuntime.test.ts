...
function addPotion(env: ReturnType<typeof setupTestEnvironment>): void {
...
describe("ConsumableRuntime regression suite", () => {
  ...
  it("B. Potion while alive: consumes potion, restores HP, triggers cooldown", () => {
    ...
    expect(env.inventoryManager.getTotalQuantity(env.heroId, "item_health_potion")).toBe(0);
  ...
  it("D. Potion while combat is stopped: rejects usage without consuming or starting cooldown", () => {
    const env = setupTestEnvironment(() => false);
    addPotion(env);
    env.damageManager.getHealth(env.heroId).currentHealth = 50;

    const result = env.consumableRuntime.useConsumable("item_health_potion");

    expect(result).toEqual({ ok: false, reason: "combat_inactive" });
    expect(env.inventoryManager.getTotalQuantity(env.heroId, "item_health_potion")).toBe(1);
