...
export function ConstructionPanel({ plotId, islandLevel, builtDefinitionIds, onBuilt }: { readonly plotId: string; readonly islandLevel: number; readonly builtDefinitionIds: ReadonlySet<IslandBuildingId>; readonly onBuilt: (definitionId: IslandBuildingId) => void; }): JSX.Element {
  ...
  const currentIslandLevel = getIslandLevelDefinition(islandLevel); const nextIslandLevel = getNextIslandLevelDefinition(islandLevel);
  ...
  if (availableBuildings.length === 0) return <section className="ui-island__selection ui-island__selection--empty"><strong>Emplacement libre</strong><p>Aucun bâtiment constructible supplémentaire n'est authoré pour cette phase.</p></section>;

  return <section className="ui-island__selection ui-island-construction">
    <div><span className="ui-island__eyebrow">Construction</span><strong>Emplacement libre</strong></div>
    ...
    <div className="ui-island-construction__list">{availableBuildings.map((definition) => {
      ...
      return <article key={definition.id} className="ui-island-construction__card">
        <header><span className="ui-island__selection-icon">{definition.icon}</span><div><strong>{definition.label}</strong><small>{definition.description}</small></div></header>
        {!categoryUnlocked && <div className="ui-island-construction__costs"><span className="is-missing">{unlockLevel === undefined ? "Non débloqué" : `Requiert île niv. ${String(unlockLevel)}`}</span></div>}
        {missingPrerequisites.length > 0 && <div className="ui-island-construction__costs">{missingPrerequisites.map((buildingId) => <span key={buildingId} className="is-missing">Requiert {getIslandBuildingDefinition(buildingId).label}</span>)}</div>}
        <div className="ui-island-construction__costs">
          ...
          {flexibleState !== undefined && <span className={flexibleAffordable ? "is-ready" : "is-missing"}>Raffinés au choix {String(flexibleAvailableTotal)} / {String(flexibleState.totalQuantity)} · {String(flexibleDistinct)} / {String(flexibleState.minimumDistinctItemIds)} familles</span>}
        </div>
        <button type="button" disabled={!affordable} onClick={() => { if (constructIslandBuilding(definition.id, plotId)) onBuilt(definition.id); }}>{!categoryUnlocked ? "Niveau d'île insuffisant" : missingPrerequisites.length > 0 ? "Prérequis manquant" : affordable ? "Construire" : "Ressources insuffisantes"}</button>
      </article>;
    })}</div>
  </section>;
}
