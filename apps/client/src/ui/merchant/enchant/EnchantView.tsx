    ...
    <div className="ui-merchant-service ui-merchant-enchant">
      <section className="ui-merchant-enchant__stocks" aria-label="Stocks d’enchantement">
        {model.stocks.map((stock) => (
          <div key={stock.itemId}>
            <ItemVisual itemId={stock.itemId} />
            <span>{stock.name}</span>
            <strong>{String(stock.quantity)}</strong>
          </div>
        ))}
      </section>

      {model.items.length === 0 ? (
        <p className="ui-merchant__empty">Aucun équipement compatible disponible.</p>
      ) : (
        <>
          <section className="ui-merchant-list" aria-label="Équipements à enchanter">
            ...
            {model.items.map((item) => (
              <button
                type="button"
                key={item.instanceId}
                className={`ui-merchant-item-row${item.instanceId === model.selectedInstanceId ? " is-selected" : ""}${getEnchantmentFrameClass(item.enchantment)}`}
                onClick={() => { setRequestedInstanceId(item.instanceId); }}
              >
                ...
                <span className="ui-merchant-item-row__identity">
                  ...
                  <small>.{String(item.enchantment)}{item.equipped ? " · Équipé" : ""}</small>
                </span>
                <b>.{String(item.enchantment)}</b>
              </button>
            ))}
          </section>
          ...
          {model.preview !== undefined && (
            <section className="ui-merchant-detail ui-merchant-enchant__detail">
              <div className="ui-merchant-detail__item">
                ...
                <div>
                  <span>Amélioration sélectionnée</span>
                  <h3>{getItemDisplayName(model.preview.itemId)}</h3>
                  <strong className="ui-merchant-enchant__level">
                    .{String(model.preview.currentLevel)} → {model.preview.nextLevel === undefined ? "MAX" : `.${String(model.preview.nextLevel)}`}
                  </strong>
                </div>
              </div>
              {model.preview.nextLevel !== undefined && (
                <div className="ui-merchant-detail__total"><span>Gain de puissance</span><strong>+{String(model.preview.itemPowerGain)} IP</strong></div>
              )}
              <dl className="ui-merchant-detail__facts">
                ...
                {model.preview.materials.map((material) => (
                  <div key={material.itemId}>
                    <dt>{material.name}</dt>
                    <dd className={material.missing > 0 ? "is-missing" : ""}>{String(material.owned)} / {String(material.required)}</dd>
                  </div>
                ))}
              </dl>
              ...
              <button
                type="button"
                className="ui-merchant__primary ui-merchant__primary--enchant"
                disabled={!model.preview.canAfford}
                onClick={() => {
                  const instanceId = actions.enchant(model.preview?.instanceId ?? "", model.incomeRate);
                  if (instanceId !== undefined) setRequestedInstanceId(instanceId);
                }}
              >
                Enchanter en .{String(model.preview.nextLevel ?? model.preview.currentLevel)}
              </button>
            </section>
          )}
        </>
