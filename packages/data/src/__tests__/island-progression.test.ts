...
describe("island operational progression", () => {
  it("maps building levels 1-5 to production tiers T3-T7", () => {
    for (const progression of ISLAND_OPERATIONAL_BUILDING_PROGRESSIONS) {
      expect(progression.levels.map((level) => level.level)).toEqual([1, 2, 3, 4, 5]);
      expect(progression.levels.map((level) => level.maxProductionTier)).toEqual([3, 4, 5, 6, 7]);
  ...
  it("charges each upgrade with the refined tier immediately below its unlock", () => {
    for (const progression of ISLAND_OPERATIONAL_BUILDING_PROGRESSIONS) {
      ...
      expect(level4Cost?.silver).toBeGreaterThan(0);
      ...
      expect(level4Cost?.requirements.length).toBeGreaterThan(0);
      ...
      expect(progression.levels[4]?.upgradeToNext).toBeUndefined();
    }
  });
});
