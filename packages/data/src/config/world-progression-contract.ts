...
export type WorldProgressionZoneRole = "progression" | "transition_plateau" | "final_gate";
...
const makeTierZones = (
...
): readonly WorldProgressionZoneContract[] => [
  {
    ...
    expected: { gearTier: tier, enchantment: 0, masteryLevel: masteryBase },
  },
  {
    zoneIndex: 1,
    role: "progression",
    expected: { gearTier: tier, enchantment: 0, masteryLevel: masteryBase + 2 },
  },
  {
    zoneIndex: 2,
    role: "progression",
    expected: { gearTier: tier, enchantment: 1, masteryLevel: masteryBase + 4 },
  },
  {
    zoneIndex: 3,
    role: "progression",
    expected: { gearTier: tier, enchantment: 2, masteryLevel: masteryBase + 7 },
  },
  {
    zoneIndex: 4,
    role: "final_gate",
    expected: { gearTier: tier, enchantment: 2, masteryLevel: masteryBase + 10 },
  },
];
...
export const WORLD_PROGRESSION_CONTRACT: Readonly<Record<WorldProgressionTier, WorldProgressionTierContract>> = {
    ...
    zones: [
      {
        zoneIndex: 3,
        role: "progression",
        expected: { gearTier: 4, enchantment: 1, masteryLevel: 25 },
      },
      {
        zoneIndex: 4,
        role: "final_gate",
        expected: { gearTier: 4, enchantment: 2, masteryLevel: 30 },
      },
    ],
  },
  ...
  8: { tier: 8, band: "black", zones: makeTierZones(8, 70) },
} as const;
...
export const WORLD_TIER_TRANSITION_CONTRACTS: Readonly<Record<WorldProgressionSourceTier, WorldTierTransitionContract>> = {
  4: {
    sourceTier: 4,
    finalZoneIndex: 4,
    nextTierFirstZoneIndex: 0,
    masteryLevel: 30,
    blockedEnchantment: 2,
    requiredEnchantment: 3,
    plateauMinSegments: 3,
    plateauMaxSegmentWithPotion: 9,
  },
  5: {
    sourceTier: 5,
    finalZoneIndex: 4,
...
export function getWorldProgressionZoneContract(
  tier: WorldProgressionTier,
  zoneIndex: number,
): WorldProgressionZoneContract {
  const zone = WORLD_PROGRESSION_CONTRACT[tier].zones.find((entry) => entry.zoneIndex === zoneIndex);
  if (zone === undefined) {
    throw new Error(`Missing world progression contract for T${String(tier)} zone index ${String(zoneIndex)}`);
  }
  return zone;
}

export function getWorldTierTransitionContract(
  sourceTier: WorldProgressionSourceTier,
): WorldTierTransitionContract {
