...
function ok<T>(value: T): AwakenedResult<T> { return { ok: true, value }; }
function fail<T>(reason: AwakenedFailureReason): AwakenedResult<T> { return { ok: false, reason }; }
...
export class AwakenedWeaponService {
  ...
  has(itemInstanceId: ItemInstanceId): boolean { return this.states.has(itemInstanceId); }
  getState(itemInstanceId: ItemInstanceId): AwakenedWeaponState | undefined { return this.states.get(itemInstanceId); }
  ...
  getDisplayTraitValue(traitId: AwakenedTraitId, value: number): number {
    if (traitId === "cooldown_reduction") return getEffectiveCooldownReductionPercent(value, this.balance);
    if (traitId === "life_steal") return getEffectiveLifeStealPercent(value, this.balance);
    return value;
  }

  listStates(): readonly AwakenedWeaponState[] { return [...this.states.values()]; }
  ...
  addAttunement(itemInstanceId: ItemInstanceId, amount: number): AwakenedResult<AwakenedAttunementGain> {
    ...
    const next = { ...state, storedAttunement: state.storedAttunement + stored };
    this.states.set(itemInstanceId, next);
    return ok({ requested: amount, stored, discardedAtCap: amount - stored, balance: next.storedAttunement, cap });
  }
  ...
  improveTrait(itemInstanceId: ItemInstanceId, traitIndex: number, walletId: WalletId, roll01: () => number): AwakenedResult<AwakenedModificationOutcome> {
    const state = this.states.get(itemInstanceId);
    if (state === undefined) return fail("weapon_not_registered");
  ...
  beginTraitOffer(itemInstanceId: ItemInstanceId, targetIndex: number, walletId: WalletId, roll01: () => number): AwakenedResult<AwakenedTraitOffer> {
    const state = this.states.get(itemInstanceId);
    if (state === undefined) return fail("weapon_not_registered");
    if (!state.awakened) return fail("weapon_not_awakened");
    if (state.pendingTraitOffer !== undefined) return fail("trait_offer_pending");
  ...
  resolveTraitOffer(itemInstanceId: ItemInstanceId, selectedTraitId?: AwakenedTraitId): AwakenedResult<AwakenedWeaponState> {
    const state = this.states.get(itemInstanceId);
    if (state === undefined) return fail("weapon_not_registered");
    if (!state.awakened) return fail("weapon_not_awakened");
    ...
    if (selectedTraitId === undefined) {
      ...
      const { pendingTraitOffer: _pending, ...rest } = state;
      this.states.set(itemInstanceId, rest);
      return ok(rest);
    }
  ...
  private spendAction(state: AwakenedWeaponState, walletId: WalletId): AwakenedResult<AwakenedActionCost> {
    ...
    const debited = this.currencyService.debit(walletId, this.options.silverCurrencyId, cost.silver, this.options.silverSpendSource);
    return debited.ok ? ok(cost) : fail("insufficient_silver");
  }
  ...
  private pickDistinctTraits(eligible: readonly AwakenedTraitId[], count: number, roll01: () => number): readonly AwakenedTraitId[] {
    const pool = [...eligible];
