...
/**
 * 34_REPAIR_SYSTEM runtime: validates, prices, debits Silver, and fully
 * restores durability. Every operation is atomic — on any failure nothing
 * changes — and duplicate transaction ids are rejected before any effect.
 */
export class RepairService {
  ...
  /** Cost preview only — never mutates any state (34_REPAIR "Server Authority"). */
  ...
  /**
   * 34_REPAIR "Bulk Repair": every damaged repairable item, worn equipment
   * first in fixed slot order then inventory positions ascending. Fully
   * atomic — if any candidate fails validation or Silver is short, nothing
   * happens.
   */
  bulkRepair(request: BulkRepairRequest): RepairResult<BulkRepairOutcome> {
    ...
    for (const slot of EQUIPMENT_SLOTS) {
      ...
      if (entry !== undefined && this.isDamaged(entry)) {
        candidates.push(entry);
      }
    }
    for (const slot of this.inventoryManager.listSlots(request.playerEntityId)) {
      if (slot.entry !== undefined && this.isDamaged(slot.entry)) {
        candidates.push(slot.entry);
      }
    }
    if (candidates.length === 0) {
      return repairFail("nothing_to_repair");
    }
  ...
  private validateContext(
    ...
    if (this.isInCombat?.(entityId) === true) {
      return repairFail("in_combat");
    }
  ...
  private validateStation(stationId: string): RepairResult<RepairStationDefinitionLike> {
    ...
    if (station === undefined) {
      return repairFail("station_not_found");
    }
    if (!station.enabled) {
      return repairFail("station_disabled");
    }
  ...
  private validateItem(
    ...
    if (this.durabilityStore.isDestroyed(instanceId)) {
      return repairFail("item_destroyed");
    }
    const info = this.resolveRepairableInfo(entry.itemId);
    const durability = this.durabilityStore.get(instanceId);
    if (info === undefined || durability === undefined) {
      return repairFail("not_repairable");
    }
    if (durability.current >= durability.max) {
      return repairFail("item_intact");
    }
    if (this.lockResolver?.(entityId, entry.itemId) === true) {
      return repairFail("item_locked");
    }
  ...
  private canAfford(walletId: BulkRepairRequest["walletId"], cost: number): RepairResult<number> {
    if (!this.currencyService.hasWallet(walletId)) {
      return repairFail("wallet_not_found");
    }
    if (cost === 0) {
      return repairOk(0);
    }
    ...
    if (!affordable.ok) {
      if (affordable.reason === "insufficient_balance") {
        return repairFail("insufficient_silver");
      }
      return repairFail("transaction_failed");
    }
  ...
  private executeRepairs(
    ...
    for (const validated of validatedList) {
      ...
      if (!restored.ok) {
        // Unreachable after validation; refund to keep atomicity anyway.
        ...
        if (totalCost > 0) {
          this.currencyService.credit(walletId, REPAIR_CURRENCY_ID, totalCost);
        }
        return repairFail("transaction_failed");
      }
      repaired.push({
  ...
  private findOwnedEntry(
    ...
    return this.inventoryManager.findEntryByInstanceId(entityId, instanceId)?.entry;
  }

  private isDamaged(entry: InventoryEntry): boolean {
    if (this.resolveRepairableInfo(entry.itemId) === undefined) {
      return false;
    }
