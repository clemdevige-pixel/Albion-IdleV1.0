...
function normalizeFileToken(value: string): string {
  return value.toLowerCase().replace(/\.png$/i, "").replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "");
}
...
function findAnimationSource(sourceDir: string, weaponId: string, animation: Animation): string | undefined {
  ...
  if (candidates.length > 1) throw new Error(`Multiple ${animation} sources found for ${weaponId}: ${candidates.join(", ")}`);
  return candidates[0] === undefined ? undefined : resolve(sourceDir, candidates[0]);
}
...
function runGenerator(args: readonly string[]): string {
  ...
  const commandArgs = npmExecPath === undefined ? ["generate:spritesheet", "--", ...args] : [npmExecPath, "generate:spritesheet", "--", ...args];
  const result = spawnSync(command, commandArgs, { cwd: REPO_ROOT, encoding: "utf-8", stdio: "pipe" });
  const stdout = result.stdout ?? "";
  const stderr = result.stderr ?? "";
  if (result.status !== 0 || result.error !== undefined) {
    const details = [result.error, stdout, stderr].filter(Boolean).map(String).join("\n").trim();
...
const sourceDir = resolve(REPO_ROOT, sourceDirRaw === undefined || sourceDirRaw.length === 0 ? DEFAULT_SOURCE_DIR : sourceDirRaw);
...
const reference = resolve(REPO_ROOT, referenceRaw === undefined || referenceRaw.length === 0 ? DEFAULT_REFERENCE : referenceRaw);
...
for (const animation of ANIMATIONS) {
  ...
  generated.push({ animation, source: toRepoRelative(source), output: toRepoRelative(output), frameCount, calibrationFrame, ...parseGeneratedMetrics(stdout) });
}

if (generated.length === 0) throw new Error(`No supported spritesheets found in ${toRepoRelative(sourceDir)} for weapon ${weaponId}.`);
