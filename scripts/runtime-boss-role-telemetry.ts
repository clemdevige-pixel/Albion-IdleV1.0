...
const rows = WORLD_ZONE_IDS_BY_BAND.blue.flatMap((zoneDefId) => WEAPONS.map((weaponItemId) => {
  const result = runCombatRuntimeBenchmark({
    label: `${String(zoneDefId)}_boss_boundary`,
    weaponItemId,
    zoneDefId,
...
console.log("[BOSS_ROLE_RUNTIME_TELEMETRY_REFERENCE]", {
  masteryLevel: MASTERY_LEVEL,
  enchantment: ENCHANTMENT,
  fullT4Armor: true,
  healthPotions: true,
...
console.log("[BOSS_ROLE_RUNTIME_TELEMETRY]");
console.table(rows.map(({ abilities: _abilities, ...row }) => row));

console.log("[BOSS_ROLE_RUNTIME_ABILITY_BREAKDOWN]");
